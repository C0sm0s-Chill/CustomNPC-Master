...
*/
        ...
        private void OnNpcDamaged(TSPlayer player, int npcIndex, int damage, float knockback, byte direction, bool critical)
        {
            ...
            double damageDone = Main.CalculateDamageNPCsTake(damage, npc.ichor ? npc.defense - 20 : npc.defense);
            if (critical)
            {
                damageDone *= 2;
            }
            ...
            if (npc.active && npc.life > 0 && damageDone >= npc.life)
            {
                ...
                if (npcvar != null)
                {
                    var economyPlayer = SEconomyPlugin.Instance.GetBankAccount(TSPlayer.Server.Account.ID);
                    ...
                    else
                    {
                        ...
                        {
                            IBankAccount Player = SEconomyPlugin.Instance.GetBankAccount(TSPlayer.Server.Account.ID);
